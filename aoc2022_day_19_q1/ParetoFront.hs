...
iMapUnzip map =
  let (keys, a1s, a2s) = unzip3 $ (\(k, (a1, a2)) -> (k, a1, a2)) <$> IMap.assocs map
   in ( IMap.fromDistinctAscList $ zip keys a1s,
      ...
      (newFront, isDominated) = case lookupRes of
        ...
        Nothing ->
          let (dominated, dominating) = splitPareto elem front
           in if not (null dominating)
                then -- if elem is dominated by any element, return the pareto front
                -- unchanged as it should not be inserted.
                  (front, True)
                else
                  if not (null dominated)
                    then -- if elem is dominating any element. We remove all dominated
                    -- elements, and insert the new element in the front.
                      (frontInsert elem Nothing $ frontDifference front dominated, False)
                    else -- Otherwise, elem is not comparable to existing elements in the
